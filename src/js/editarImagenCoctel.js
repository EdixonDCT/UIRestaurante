...
const enviarImagen = async (e) => {
  ...
  try {
    ...
    if (!resDelete.ok) {
      ...
      throw new Error(msgDelete || "Error al eliminar la imagen anterior.");
    }
    ...
    if (!resUpload.ok || !json.url) {
      throw new Error(json.error || "Error al subir la nueva imagen.");
    }

    // Actualizar imagen en el cóctel
    const imagen = { imagen: json.nombre };

    const actualizar = await fetch(`http://localhost:8080/ApiRestaurente/api/cocteles/imagen/${idCoctel}`, {
      method: "PATCH",
      headers: {
